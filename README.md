# Hi there, I'm Martins! 👋

**Full-Stack Software Engineer | System Architecture | AI Integration**

Welcome to my Github Profile!

I am a product-minded engineer and MS in Electrical & Computer Engineering candidate at CU Boulder. I specialize in designing, building, and deploying scalable web applications with a focus on high-concurrency database design, complex state management, and modern frontends.

🌍 **[Visit My Full Portfolio Website](https://marrtinerz.github.io)**
🔗 **[Connect with me on LinkedIn](https://www.linkedin.com/in/mnnamchi/)**
✉️ **[Reach out via Email1](mailto:mnnamchi@gmail.com)**
✉️ **[Reach out via Email2](mailto:martins.nnamchi@colorado.edu)**

---

### 🚀 What I'm currently working on
- Expanding **ShelterTree**, a pan-African property intelligence platform using Python, Django, PostgreSQL, and HTMX.
- Deepening my knowledge in **Data Structures & Algorithms** and Semiconductor Physics at CU Boulder.
- Building AI-integrated tools utilizing LLM APIs and prompt engineering.

### 🛠️ Tech Stack & Tools
- **Backend:** Python, Django, Flask, RESTful APIs
- **Frontend:** JavaScript (ES6+), HTMX, HTML5, CSS3, Bootstrap 5
- **Databases:** PostgreSQL, SQL, Redis
- **Infrastructure:** Docker, Render, AWS S3, Git, CI/CD
- **AI & Data:** Gemini API, Pandas, Scikit-Learn

---

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Marrtinerz&layout=compact&theme=vision-friendly-dark" />
</p>
